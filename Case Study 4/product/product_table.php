<?php
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
      ...
      if ($dbl) { // displays only if dbl price is not NULL
        $dbl = number_format($dbl, 2, '.', ''); // 2 decimal places
        echo " Double: $";
        echo "<span class='priceDisplay'>".$dbl."</span>"; // dbl price
        echo " <input name='dbl' value='".$dbl."' class='priceInput' type='number' min='0.01' step='0.01'>"; // dbl input
      }
